...
class AsBuilt(object):
	...
	def start_byte(self, block):
		return sum(self.fieldsizes[:block-1])
...
class HmiData(object):
	...
	items_de08 = [
		...
		{ 	'name': 'TIRE_MOBILITY_KIT Support note, bit is probably wrong',
			'index': 218,
			'byte': 10,
			'bit': 2, # ?
			'size': 1,
		...
		for i in range(self.size()):
			...
			if self.is_table(i):
				...
				if hasattr(JumpTables, self.value(i).replace("___", "_").replace("__", "_")):
					...
					#string = string + "".join(dir(JumpTables))
				continue
			for x in range(0, len(self.value(i))):
				#bitsor = (bitmask(bit, i['items'] - 1) & data[loc])
				#bitsor2 = (bitmask(bit, i['items'] - 1) & data2[loc]) if data2 is not None else None
				#bits = bitsor >> (8 - (bit % 8)) if (bit % 8) / 8.0 != 0.0 else bitsor
				#bits2 = bitsor2 >> (8 - (bit % 8)) if data2 is not None and (bit % 8) / 8.0 != 0.0 else bitsor2
				#print("%02X %02X %02X %02X %d" % (bitmask(bit, i['items'] - 1), bitsor, bits, data[loc], bit % 8))
	...
	def format_de4_6(self, ab1, ab2=None):
		...
		for i in range(0, len(self.high_items)):
			if '255' in self.high_items[i]:
				...
				string = string + "%-48s%-32s\t%s%s\t%-16s%-16s\tMin: %0.1f\tMax: %0.1f\n" %(
	...
	def format_de7(self, ab1, ab2=None):
		return ""

	def format_de8(self, ab1, ab2=None):
		return ""
	...
	def format(self, ab1, ab2=None):
		...
		string = string + self.format_de7(ab1, ab2)
		string = string + self.format_de8(ab1, ab2)
		return string
...
def print_bits_known_de07_08():
	...
	for i in items.items_de08:
		if i['size'] == 1:
			de8[((i['byte']-1) * 8) + 7 - i['bit']] = 1
		else:
			x = ((i['byte']-1) * 8) + 7 - i['bit']
			de8 = de8[:x] + [1] * i['size'].bit_length() + de8[x+i['size'].bit_length():]
	c = 0
	print("Known bits in DE07 // 7D0-08-xx")
	for x in de8:
		if c % 8 == 0 and c > 0:
...
"""
	...
	if abtfile is not None and len(abtfile) > 0 and not debug:
		asbuilt1 = AsBuilt(abtfile[0])
